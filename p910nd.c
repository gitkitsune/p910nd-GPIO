 ...
 *
 *	Version 0.97-gpio
 *	Patch by Oleg Artamonov modified by Jaroslaw Strauchmann
...
static char version[] = "Version 0.97-gpio";
...
static int gpioNum = 408;
static int gpioTimeout = 120;
...
void gpioSet(int gpio, int value)
{
    ...
    write(fd, buf, 1);
    close(fd);
    printf("Zmiana gpio%d na stan %d\n", gpio, value);
}
...
void usage(void)
{
	...
	fprintf(stderr, "Usage: %s [-f device] [-i bindaddr] [-g GPIO] [-t timeout] [-bvd] [0|1|2]\n", progname);
	exit(1);
}
...
int main(int argc, char *argv[])
{
	...
	lpnumber = '0';
	while ((c = getopt(argc, argv, "bdi:f:g:t:v")) != EOF) {
		switch (c) {
		case 'b':
			bidir = 1;
			break;
